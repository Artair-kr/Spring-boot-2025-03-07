...
// User 테이블에 접근할 리포지토리
// JpaRepository<테이블이 저장된 엔티티명, PK 타입>
@Repository
public interface UserRepository extends JpaRepository<UserEntity, String>{
    ...
    // Quert Methid:
    // - JpaRepository의 메서드 선언시에 지정된 규칙에 따라 메서드명을 작성하면 
    // jPA가 SQL을 만들어주는 방법

    // - findBy : [SELECT * WHERE]: 필드명을 기준으로 모든 컬럼을 조회할 때 사용한다.
    // findBy 바로 뒤에 필드명을 붙여서 작성, 필드명은 반드시 첫글자가 대문자로 작성되어야한다.
    // SELECT * FROM user WHERE user_name ?;
    // List 타입으로 반환받는 경우 결과가 없으면 길이가 0인 리스트를 반환한다.
    List<UserEntity> findByUserName(String userName);

    // existBy: 조건에 해당하는 레코드가 존재하는지 여부 확인시 사용한다.
    // boolean 형태로 반환
    ...
    // And / Or : and 연산 혹은 or 연산에 사용된다, 필드와 필드 사이 작성
    // And Or 연산 우선순위 주의 And가 Or 보다 우선순위가 높다.
    ...
    // OrderBy: 정렬을 사용할 필드를 지정해서 Asc, Desc 시에 사용한다.
    //SELECT * FROM user ORDER BY user_name DESC;
    List<UserEntity> findByOrderByUserNameDesc();

    // deleteBy: 조건에 따른 레코드를 삭제할 때 사용
    // @Transactional 을 적용해야 사용가능
    @Transactional
    void deleteByUserName(String userName);

    // @Query:
    // - 쿼리 메서드 방식의 한계를 극복하기위해 사용되는 방법
    // - 복잡한 쿼리 (서브쿼리, 조인, 그룹화)를 직접 작성하는 방법

    // JPQL (Java Persistance Query Language)
    // - 표준 SQL과 매우 흡사하지만 Entity 및 Entitiy의 필드로 쿼리를 작성한다.
    //  "SELECT * FROM user WHERE user_Name = ?"
    // SELECT u.userId FROM / SELECT u FROM 
    // ?1 매개변수 1번째 값 / ?2 매개변수 2번째 값
    @Query(value = "SELECT u.userId FROM user u WHERE u.userName = ?1 AND u.userTelNumber = ?2")
    List<UserEntity> getUserList1(String userName, String userTelNumber);

    @Query(value = "SELECT u.userId FROM user u WHERE u.userName = :name AND u.userTelNumber = :telNumber")
    ...
    // - @Query의 nativeQuery 속성을 반드시 true로 지정해야 사용이 가능하다.
