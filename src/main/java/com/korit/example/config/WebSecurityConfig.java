package com.korit.example.config;
...
public class WebSecurityConfig {
    ...
    // @Bean:
    // @Component를 사용하지 못하거나 사용하고 싶지 않을때 Spring bean으로
    // 등록할 수 있도록 하는 어노테이션
    // WebSecurityConfig 클래스에 인스턴스 생성을 Spring에게 넘기지 않고 
    // configure 메서드 호출만 Spring에게 넘기는 것
    @Bean
    protected SecurityFilterChain configure(HttpSecurity security) 
    throws Exception {

        // Class::method
        // - 메서드 참조,특정 클래스의 메서드를 참조할 때 사용한다.
        // - 매개변수로 메서드를 전달하고자 할때 자두 사용한다.
        security
          ...
          // Session:
          // 웹 애플리케이션에서 클라이언트에 댛나 정보를 유지하기 위한 기술
          // 서버측에서 클라이언트에 대한 정보를 유지하는 방법
          // REST API 서버에서는 Session을 유지하지 않는다.

          // Session 유지 방식에 대한 설정
          // Sessoin 유지를 하지 않겠다고 지정
          .sessionManagement(sessionManagement -> sessionManagement.sessionCreationPolicy(SessionCreationPolicy.STATELESS))

          // CSRF (Cross-Site Request Forgery)
          // - 클라이언트가 자신의 의도와는 무관하게 공격행위를 하는 것
          // - 흔히 공격자가 정당한 클라이언트의 세션을 탈취하여 공격을 수행한다.
          ...
          // 인가 작업
          // - 요청 URL의 패턴(자원)에 따라 인가가 필요한 작업인지 지정하는 설정
          // (자원에 대한 권한 지정)
          // - 모든 클라이언트가  접근할 수 있도록 허용
          // - 인증된 모든 클라이언트가 접근할 수 있도록 허용
          // - 인증된 클라이언트 중 특정 권한을 가진 클라리언트만 접근할 수 있도록 허용
          .authorizeHttpRequests(request -> request
            ...
            // (매개변수로 전달하는 권한명은 ROLE_를 제거한 실제 권한명)
